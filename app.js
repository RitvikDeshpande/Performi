...
// mongoose.connect("mongodb+srv://Ritvik:Rlpie689$@cluster1.crimt.mongodb.net/Cluster1?retryWrites=true&w=majority", {useNewUrlParser: true});
...
function seedDB () {
    ...
    User.register(demo_user, password, function(err, demoUser){
        ...
        Manager.create(demoAdmin, function(err, manager){
            if(err){
                console.log(err);
            }else{
                manager.userId = demoUser._id;
                manager.save();
                demoUser.manager = manager;
                demoUser.save();
                app.locals.currentUser3 = manager;
            }
        });
